...
function wordmark(opts = {}) {
  ...
  return `<svg class="wordmark" height="${height}" viewBox="-16 -25 470 184" role="img" aria-label="Gizmoz">
  <circle cx="42.85" cy="42.85" r="30.4" fill="none" stroke="${ringColor}" stroke-width="24.85"/>
  <circle cx="102.35" cy="-3.55" r="17.9" fill="${accent}"/>
  <path d="${SLASH_PATH}" fill="${accent}"/>
  <text x="96" y="122" font-family="Poppins, Segoe UI, sans-serif" font-size="136" font-weight="700" fill="${ringColor}" letter-spacing="-2">izmoz</text>
</svg>`;
}
