...
/** The tail, positioned for the full lockup (ring centre 73, 56.81). */
const TAIL_LOCKUP =
  'M 9.62 154.57 L 136.38 120.53 C 138.17 120.53 139.62 122.03 139.62 123.87 L 139.62 153.40 C 139.62 155.24 138.17 156.74 136.38 156.74 L 9.62 190.78 C 7.83 190.78 6.38 189.28 6.38 187.44 L 6.38 157.91 C 6.38 156.07 7.83 154.57 9.62 154.57 Z';
...
/**
 * The full logo lockup: the "g", the word, and the tagline set beneath it —
 * the same arrangement as the supplied brand artwork, drawn as vector rather
 * than placed as a bitmap so it stays sharp at any size, carries no
 * background of its own, and recolours with CSS.
 *
 * Both runs of text pin their advance width with `textLength`, so neither can
 * overrun the viewBox if a fallback font renders wider than Poppins.
 */
function wordmark(opts = {}) {
  ...
  return `<svg class="wordmark" height="${height}" viewBox="0 -31 471 229" role="img"
     aria-label="Gizmoz \u2014 ${tagline}">
  <circle cx="73" cy="56.81" r="35.73" fill="none" stroke="${ringColor}" stroke-width="32.18"/>
  <circle cx="130" cy="-7.45" r="16.32" fill="${accent}"/>
  <path d="${TAIL_LOCKUP}" fill="${accent}"/>
  <text x="165.51" y="100" textLength="298.3" lengthAdjust="spacingAndGlyphs"
        font-family="Poppins, Segoe UI, sans-serif" font-size="100" font-weight="700"
        fill="${ringColor}">izmoz</text>
  <text x="165.51" y="128.7" textLength="261.8" lengthAdjust="spacingAndGlyphs"
        font-family="Poppins, Segoe UI, sans-serif" font-size="15.4" font-weight="400"
        fill="${taglineColor}">${tagline}</text>
</svg>`;
}
